...
describe("MessageItem", () => {
  ...
  const renderMessage = (
    messageOverrides = {},
    options = { isSelf: false, isRepeated: false },
  ) =>
  ...
  describe("message menu", () => {
    it("shows menu button only for own messages", () => {
      ...
      expect(
        screen.getByRole("button", { name: /message options/i }),
      ).toBeInTheDocument();
    });
    ...
    it("hides menu button for other users messages", () => {
      ...
      expect(
        screen.queryByRole("button", { name: /message options/i }),
  ...
  describe("editing", () => {
    it("calls sendEditedMessage when editing and saving", async () => {
      ...
      expect(mockChatService.sendEditedMessage).toHaveBeenCalledWith(
        "m1",
        "Updated message",
      );
    });
