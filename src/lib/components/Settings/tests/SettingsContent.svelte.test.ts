...
describe("SettingsContent", () => {
  ...
  it("displays current email in profile tab", () => {
    renderContent("profile");
    expect(
      screen.getByText(/Current Email: john@example.com/),
    ).toBeInTheDocument();
  });
  ...
  it("displays garden name in garden tab for owner", () => {
    renderContent("garden");
    expect(
      screen.getByText(/Current Garden Name: My Garden/),
    ).toBeInTheDocument();
  });
  ...
  it("renders ProfileSettings component when tab is active", () => {
    renderContent("profile");
    expect(
      screen.getAllByText(/Change Username|Change Email/i).length,
    ).toBeGreaterThan(0);
  });
