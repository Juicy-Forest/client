...
<div class="flex items-center justify-between rounded-xl border border-stone-200 bg-white px-4 py-3 transition-colors hover:bg-stone-50/80">
    <div class="flex items-center gap-3 overflow-hidden">
        ...
        <div class="flex flex-col min-w-0">
            <p class={`font-semibold text-sm ${task.isComplete ? 'text-stone-400 line-through' : 'text-stone-800'}`}>
                {task.name}
            </p>
            {#if task.description}
                <div class="relative">
                    <p class={`text-xs leading-relaxed transition-all duration-300 ${isExpanded ? '' : 'max-h-8 overflow-hidden'} ${task.isComplete ? 'text-stone-400' : 'text-stone-500'}`}>
                        {task.description}
                    </p>
