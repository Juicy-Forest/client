...
describe('login action', () => {
  ...
  it('should fail with 400 if email is missing', async () => {
    mockRequest.formData.mockResolvedValue(new Map([['password', 'password123']]));
  ...
  it('should fail with 400 if password is missing', async () => {
    mockRequest.formData.mockResolvedValue(new Map([['email', 'test@example.com']]));
  ...
  it('should handle successful login and redirect', async () => {
    ...
    try {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3030/users/login', {
        method: 'POST',
        headers: {
