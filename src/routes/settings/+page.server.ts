...
export const actions: Actions = {
  ...
  updateUsername: async ({ request, cookies, fetch }) => {
    ...
    try {
      ...
      if (!response.ok) {
        return fail(response.status, {
          error: result.error || "Username change failed",
        });
      }
  ...
  updateGardenName: async ({ request, cookies, fetch }) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errorMessage =
          result.message || result.error || "Garden name change failed";
      ...
      return {
        success: true,
        message: "Garden name changed successfully!",
        newGardenName,
      };
    } catch (error) {
  ...
  removeMember: async ({ request, cookies, fetch }) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/garden/${gardenId}/removeMember`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
